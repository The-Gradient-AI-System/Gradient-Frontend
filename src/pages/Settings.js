...
const Settings = () => {
  const [prompts, setPrompts] = useState({ follow_up: '', recap: '', quick: '' });
  ...
  const resetPrompts = async () => {
    ...
    try {
      ...
      setPrompts(fresh || { follow_up: '', recap: '', quick: '' });
  ...
  return (
    <PageWrapper>
      <SettingsPanel>
        ...
        <PromptGrid>
          <PromptSection>
            <PromptLabel>Quick Reply (швидка відповідь)</PromptLabel>
            <PromptHint>Дуже коротка, неформальна відповідь (до 60 слів).</PromptHint>
            <PromptEditor
              value={prompts.quick}
