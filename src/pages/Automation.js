...
const ReplyTextareaAttachBtn = styled.button`
  position: absolute;
  bottom: 1rem;
  right: 1rem;
  width: 40px;
...
const ReplyStyleRow = styled.div`
  display: flex;
  align-items: center;
  gap: 0.6rem;
  margin-bottom: 1rem;
...
const ReplyStyleLabel = styled.span`
  font-size: 0.95rem;
  color: ${({ theme }) => theme.colors.subtleText};
  white-space: nowrap;
`;

const ReplyStylePill = styled.button`
  border-radius: 999px;
...
const ReplyAttachRow = styled.div`
  display: flex;
  align-items: center;
  gap: 0.75rem;
  margin-top: 0.75rem;
...
const AttachedFileCard = styled.span`
  display: inline-flex;
  flex-direction: column;
  align-items: center;
  gap: 0.5rem;
  ...
  .file-icon-wrap {
    ...
    height: 44px;
    border-radius: 12px;
    background: rgba(104, 125, 255, 0.15);
    color: ${({ theme }) => theme.colors.primary || '#6b84ff'};
  }
...
const REPLY_TYPE_KEYS = ['quick', 'follow_up', 'recap'];
const REPLY_TYPE_LABELS = {
  quick: 'Quick reply',
  follow_up: 'Follow-up',
  recap: 'Recap & Proposal',
};
const REPLY_STYLE_KEYS = ['official', 'semi'];
const REPLY_STYLE_LABELS = {
  official: 'Official',
  semi: 'Semi-official',
};
const EMPTY_REPLY_OPTIONS = REPLY_SIX_KEYS.reduce((acc, k) => ({ ...acc, [k]: '' }), {});
...
const Automation = () => {
  ...
  const [replyOptions, setReplyOptions] = useState(EMPTY_REPLY_OPTIONS);
  ...
  const sortedLeads = useMemo(() => {
    return [...filteredLeads].sort((a, b) => {
      const priorityOrder = (p) => (p === 'high' ? 0 : 1);
      const pa = priorityOrder(a.priority);
      const pb = priorityOrder(b.priority);
      if (pa !== pb) return pa - pb;
      const da = parseDate(a.received_at)?.getTime() ?? 0;
      const db = parseDate(b.received_at)?.getTime() ?? 0;
      return db - da;
    });
  }, [filteredLeads]);
  ...
  const closeModal = useCallback(() => {
    ...
    setReplyOptions(EMPTY_REPLY_OPTIONS);
    setSelectedReplyKey('');
    setReplyDraft('');
    setReplyError(null);
  }, []);
  ...
  const selectedReplyType = selectedReplyKey ? (selectedReplyKey.split('_')[0] || '') : '';
  const selectedReplyStyle = selectedReplyKey && selectedReplyKey.includes('_') ? selectedReplyKey.replace(/^[^_]+_/, '') : '';

  const handleSelectReplyType = (type) => {
    const style = selectedReplyStyle || REPLY_STYLE_KEYS[0];
    const key = `${type}_${style}`;
    setSelectedReplyKey(key);
    setReplyDraft(replyOptions[key] || '');
  };

  const handleSelectReplyStyle = (style) => {
    const type = selectedReplyType || REPLY_TYPE_KEYS[0];
    const key = `${type}_${style}`;
    setSelectedReplyKey(key);
    setReplyDraft(replyOptions[key] || '');
  };
  ...
  const handleGenerateReplies = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        lead: selectedLead,
        tone: selectedLead.tone || undefined,
      };
      const response = await postGenerateReplies(payload);
      const replies = response?.replies || {};
      const normalizedReplies = REPLY_SIX_KEYS.reduce(
        (acc, key) => ({ ...acc, [key]: typeof replies[key] === 'string' ? replies[key] : '' }),
        {}
      );

      setReplyOptions(normalizedReplies);

      const populatedKeys = REPLY_SIX_KEYS.filter((key) => (normalizedReplies[key] || '').trim());
                ...
                {sortedLeads.map((lead, index) => {
                  ...
                  return (
                    <tr
                      ...
                      style={rowStyle}
                      data-is-lead={lead.is_lead ? 'true' : 'false'}
                    >
                      <td>
            ...
            <ModalMain>
              ...
              <ModalHeader>
                ...
                <ModalMeta>
                  ...
                  {(selectedLead.tone || selectedLead.status_label) && (
                    <MetaLine>
                      <MetaLabel>Характер листа</MetaLabel>
                      <MetaValue>{selectedLead.tone || selectedLead.status_label || '—'}</MetaValue>
                    </MetaLine>
                  )}
                </ModalMeta>
              </ModalHeader>
                        ...
                        <>
                          <InsightSection>
                            ...
                            <InfoGrid>
                              ...
                              <InfoRow>
                                ...
                                <InfoValue>{selectedInsights?.phone_number || selectedInsights?.phone || selectedLead?.phone || '—'}</InfoValue>
                              </InfoRow>
                            </InfoGrid>
                            <SummaryBlock>
                            ...
                            {!!selectedInsights?.person_links?.length && (
                              <TagRow>
                                {selectedInsights.person_links.slice(0, 2).map((link) => (
                            ...
                            {!!selectedInsights?.person_insights?.length && (
                              <SearchResults>
                                {selectedInsights.person_insights.slice(0, 2).map((item, index) => (
                          ...
                          <InsightSection>
                            ...
                            {!!selectedCompanyInsights.length && (
                              <SearchResults>
                                {selectedCompanyInsights.slice(0, 2).map((entry, index) => (
      ...
      {showReplyComposer && (
        <ReplyComposerOverlay onClick={closeReplyComposer}>
          <ReplyComposerContent onClick={(event) => event.stopPropagation()}>
            ...
            <ReplyStyleRow>
              <ReplyStyleLabel>Type</ReplyStyleLabel>
              {REPLY_TYPE_KEYS.map((typeKey) => {
                const hasContent = (replyOptions[`${typeKey}_official`] || '').trim() || (replyOptions[`${typeKey}_semi`] || '').trim();
                return (
                  <ReplyVariantButton
                    key={typeKey}
                    type="button"
                    $active={selectedReplyType === typeKey}
                    onClick={() => handleSelectReplyType(typeKey)}
                    disabled={!hasContent}
                  >
                    {REPLY_TYPE_LABELS[typeKey]}
                  </ReplyVariantButton>
                );
              })}
            </ReplyStyleRow>
            <ReplyStyleRow>
              <ReplyStyleLabel>Style</ReplyStyleLabel>
              {REPLY_STYLE_KEYS.map((styleKey) => {
                const key = `${selectedReplyType || 'quick'}_${styleKey}`;
                const hasContent = (replyOptions[key] || '').trim();
                return (
                  <ReplyStylePill
                    key={styleKey}
                    type="button"
                    $active={selectedReplyStyle === styleKey}
                    onClick={() => handleSelectReplyStyle(styleKey)}
                    disabled={!hasContent}
                  >
                    {REPLY_STYLE_LABELS[styleKey]}
                  </ReplyStylePill>
                );
              })}
            </ReplyStyleRow>
            <SectionHint>
              {selectedReplyKey?.startsWith('quick')
                ? 'Дуже коротка відповідь (до 60 слів).'
                : selectedReplyKey?.startsWith('follow_up')
                ? 'Варіант м’якого фолоу-апу після знайомства.'
                : selectedReplyKey?.startsWith('recap')
            ...
            {attachedFiles.length > 0 && (
              <ReplyAttachRow>
                {attachedFiles.map((file, index) => (
                  <AttachedFileCard key={`${file.name}-${index}`}>
                    <button type="button" onClick={() => removeAttachedFile(index)} aria-label="Видалити файл">
                      <FiX size={12} />
                    </button>
            ...
            <ReplyComposerActions>
              <ReplyComposerButton type="button" onClick={handleCopyDraft} disabled={!replyDraft.trim()} title="Скопіювати текст">
                Скопіювати
              </ReplyComposerButton>
